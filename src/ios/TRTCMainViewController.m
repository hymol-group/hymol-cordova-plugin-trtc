...
- (void)initUI {
    ...
    // 如果角色是医生
    if (self.role == 0) {
        NSInteger x = self.view.width / 2 - 40;
        NSInteger y = self.view.height - 100;
        UIButton *btnHangUp = [self createBottomBtnIcon:@"hangup" action:@selector(clickHangUp:)];
        [btnHangUp setFrame:CGRectMake(x, y, 80, 80)];
    } else {
         NSInteger barHeight = [[UIApplication sharedApplication] statusBarFrame].size.height;
        
        UIView *docView = [[UIView alloc] initWithFrame:CGRectMake(0, barHeight + 20, self.view.width, 220)];
        [docView setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:.1]];
        UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake(5, 15, 100, 100)];
        img.image = self.physicianImage;
        //[[UIImageView alloc] initWithImage:self.physicianImage];        
        // 下载网络图片
...
//        });
        ...
        // 名号
        NSString *titleName = self.physicianRole;
        NSString *name = self.physicianName;
        NSString *desc = self.physicianDesc;
        
        UIFont *font = [UIFont systemFontOfSize:18];
        UIFont *nameFont = [UIFont systemFontOfSize:20];
        UIFont *descFont = [UIFont systemFontOfSize:14];
        CGSize titleSize = [titleName sizeWithAttributes: @{NSFontAttributeName: font}];
        CGSize nameSize = [name sizeWithAttributes:@{NSFontAttributeName: nameFont}];
        CGSize descSize = [desc boundingRectWithSize:CGSizeMake(self.view.width - 115, MAXFLOAT)
                                             options:NSStringDrawingUsesLineFragmentOrigin
                                          attributes:@{NSFontAttributeName: descFont} context:nil].size;
        
        [docView setSize:CGSizeMake(docView.size.width, MAX(115 , 90 + descSize.height + titleSize.height))];
        
        UILabel *txtTitle = [[UILabel alloc] initWithFrame:CGRectMake(135 + nameSize.width, 10, titleSize.width+10, titleSize.height+5)];
        [txtTitle setText: titleName];
        [txtTitle setTextColor:UIColor.whiteColor];
        [txtTitle setFont:font];
        txtTitle.layer.borderColor = [UIColor whiteColor].CGColor;
        txtTitle.layer.cornerRadius = 4;
        txtTitle.layer.borderWidth = 1;
        txtTitle.textAlignment = NSTextAlignmentCenter;
        
        UILabel *txtName = [[UILabel alloc] initWithFrame:CGRectMake(125, 10, nameSize.width, nameSize.height)];
        [txtName setText:name];
        [txtName setFont:nameFont];
        [txtName setTextColor:UIColor.whiteColor];
        UILabel *txtDesc = [[UILabel alloc] initWithFrame:CGRectMake(125, 30 + nameSize.height, descSize.width, descSize.height)];
        [txtDesc setTextColor:UIColor.whiteColor];
        txtDesc.numberOfLines = 0;
        [txtDesc setText: desc];
        [txtDesc setFont:descFont];
        
        [docView addSubview:img];
        [docView addSubview:txtTitle];
        [docView addSubview:txtName];
        [docView addSubview:txtDesc];
        [self.view addSubview:docView];
    }
}

-(void)hangUp{
    [self clickHangUp:nil];
}

// 挂断视频
-(void)clickHangUp: (UIButton*)btn {
    [self exitRoom];
