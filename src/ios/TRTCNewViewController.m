...
@property (nonatomic, retain) UISegmentedControl* customRoleSeg;
...
//    [self.view addSubview:customVideoCaptureLabel];
    ...
    _customRoleSeg = [[UISegmentedControl alloc] initWithItems:@[@"医生", @"患者"]];
    [_customRoleSeg setTitleTextAttributes:attributes
                                          forState:UIControlStateNormal];
    _customRoleSeg.bounds = CGRectMake(0, 0, self.view.width * 0.4, 35);
    _customRoleSeg.center = CGPointMake(self.view.width - _customRoleSeg.width / 2 - 10, _customVideoCaptureSeg.bottom + 45);
    _customRoleSeg.tintColor = UIColorFromRGB(0x05a764);
    _customRoleSeg.selectedSegmentIndex = 0;
    [_customRoleSeg setTitleTextAttributes:@{NSForegroundColorAttributeName:UIColor.whiteColor} forState:UIControlStateSelected];
    [_customRoleSeg setTitleTextAttributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0x939393)} forState:UIControlStateNormal];
    [self.view addSubview:_customRoleSeg];

    
    UILabel *customRoleLabel = [[UILabel alloc] init];
    customRoleLabel.textColor = userTipLabel.textColor;
    customRoleLabel.text = @"用户角色";
    [customRoleLabel sizeToFit];
    customRoleLabel.center = CGPointMake(customVideoCaptureLabel.x + customRoleLabel.width / 2, _customRoleSeg.center.y);
    [self.view addSubview:customRoleLabel];
...
//    param.bussInfo = @"{\"Str_uc_params\":{\"pure_audio_push_mod\":1}}"; //纯音频推流参数设置示例
    ...
    vc.role = _customRoleSeg.selectedSegmentIndex;
    vc.param = param;
